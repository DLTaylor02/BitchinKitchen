...
elseif($path==='/register' && method_is('POST')) { Csrf::verify(); $name=trim($_POST['name']??'');$pass=$_POST['password']??''; if(strlen($name)<2||strlen($name)>100||strlen($pass)<8){flash('Use a username of 2–100 characters and a password of at least 8 characters.','error');redirect('/register');} try{$q=Database::connection()->prepare("INSERT INTO users(name,password_hash,role)VALUES(?,?,'user') RETURNING id");$q->execute([$name,password_hash($pass,PASSWORD_DEFAULT)]);$_SESSION['user_id']=$q->fetchColumn();redirect('/');}catch(Throwable){flash('That username is already registered.','error');redirect('/register');} }
